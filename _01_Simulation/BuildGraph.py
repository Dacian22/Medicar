...
def closest_node_to_special_nodes(special_nodes, nds, named_nodes):
    ...
    for node_id, name in [item for item in named_nodes.items() if item[1] in special_nodes]:
        ...
        for nd_id in nodes_to_check.keys():
            ...
            dist = (float(nds[nd_id]['x']) - float(nds[node_id]['x'])) ** 2 + (
                    float(nds[nd_id]['y']) - float(nds[node_id]['y'])) ** 2
...
def set_weights_to_inf(G, edges_to_be_set_to_inf):
    ...
    if edges_to_be_set_to_inf is None:
        ...
        for edge in G.edges():
            if (str(edge[0]) == str(edges_to_be_set_to_inf[0]) and str(edge[1]) == str(edges_to_be_set_to_inf[1])) or (
                    str(edge[0]) == str(edges_to_be_set_to_inf[1]) and str(edge[1]) == str(edges_to_be_set_to_inf[0])):
...
def set_weight_to_value(G, edge_to_be_set_to_inf, value_to_set, method):  # only single edge # method: minutes, factor
    ...
    if edge_to_be_set_to_inf is None:
        ...
        for edge in G.edges():
            if (str(edge[0]) == str(edge_to_be_set_to_inf[0]) and str(edge[1]) == str(edge_to_be_set_to_inf[1])) or (
                    str(edge[0]) == str(edge_to_be_set_to_inf[1]) and str(edge[1]) == str(edge_to_be_set_to_inf[0])):
